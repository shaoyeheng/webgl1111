            ...
            handler.setInputAction(function(click) {
               ...
               if (pick && pick.id) {
                  engine.viewer.entities.values.map((item)=>{
                     ...
                     if(pick.id && (item.id == pick.primitive.id.id)){
                        let url = './merf-qizhong/renderer/renderer.html?dir=../data/zkd220&quality=medium'
                        if(item.id == 'point1'){
                           url = './merf-qizhong/renderer/renderer.html?dir=../data/assets&quality=medium'
                        }
            ...
            //从Cesium中获取当前地图瓦片等级
               ...
               if (Cesium.defined(tilesToRender)) {
                  ...
                  if(height < 1500000 && !point1){
                     ...
                     point2 = engine.viewer.entities.add({
                        id:'point2',
                        position : new Cesium.Cartesian3.fromDegrees(117.257063,31.840371, 0),
                        point : {
                           pixelSize : 16,//点的大小
                           color: Cesium.Color.YELLOW,//点的颜色
                           outlineWidth:2,//边框宽度
                     ...
                     cameraOccluder.isPointVisible(point1);
                     cameraOccluder.isPointVisible(point2);
                  ...
                  if(height > 1500000 && point1){
                     ...
                     engine.viewer.entities.remove(point2)
                     point2 = null
                  }

                  if(height < 1000 && !isOpen){
                     this.openPage('./merf-qizhong/renderer/renderer.html?dir=../data/zkd220&quality=medium')
                     isOpen = true
                  }
